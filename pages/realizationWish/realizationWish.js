...
console.log(that.data.claimant);
    if(that.data.claimant === null ||that.data.claimant ==='' ){
      //app.toast('请输入认领人')
      //return 
      this.setData({
        claimant: that.data.userInfo.nick
      })
    }
    console.log('----'+that.data.claimant);
    // use api
    wx.request({
      url: app.globalData.webroot + 'xcx/api/realizationWish', //上线的话必须是https，没有appId的本地请求貌似不受影响 
      ...
      data: {
        pid:that.data.pid,
        wishid:that.data.pid,
        claimant:that.data.claimant,
        claimantid:that.data.userInfo.pid,
        realize_time:that.data.time,
        expressage:that.data.expressage,
